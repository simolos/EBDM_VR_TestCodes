...
try:
    ...
    print("EP phase")
    streamer.send_event("EPphase", {"dur_EPphase": round(dur_EPphase, 2), "cursor_pos": round(opening_percentage, 2)})

    # Simulate real door opening 
    freq = 100 # Hz (screen sampling rate)
    ...
    opening_percentage = np.clip(base_values + noise, 0, 1)  # keep between 0 and 1

    for i, val in enumerate(opening_percentage):

        streamer.send_event("EPphase", {"dur_EPphase": round(dur_EPphase, 2), "cursor_pos": round(val, 2)}) # dur_EPphase - i/freq

        # Wait for 1/freq seconds
        time.sleep(1/freq)
